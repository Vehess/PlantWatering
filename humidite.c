 ...
 * Code original issu du lien suivant, et modifié pour mes besoins
...
typedef enum{
	...
	DHT_TIMEOUT_ERROR  = 1,  // Temps d'attente dépassé
	DHT_CHECKSUM_ERROR = 2   // Données reçues erronées
}DHT_t;
...
void humidite_engine()
{
	...
	switch (readDHT11(BROCHE_CAPTEUR, &temperature, &humidity))
	{
		case DHT_SUCCESS:
			...
			//Penser a récuperer l'information ici.
